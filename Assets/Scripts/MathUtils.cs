...
{
	public static void DrawCircle(Vector2 position, float radius, int segments =  32)
	{
		float angleStep = 360f / segments;
		for (int i = 0; i < segments; i++)
		{
			float theta = (i * angleStep) * Mathf.Deg2Rad;
			float nextTheta = ((i + 1) * angleStep) * Mathf.Deg2Rad;
			...
			Vector3 end = new Vector3(
				position.x + Mathf.Cos(nextTheta) * radius,
				position.y + Mathf.Sin(nextTheta) * radius,
				0f);
